...
class PersistentMusicPlayer {
    constructor() {
        ...
        // Initialize base path for assets
    ...
    // Method to get asset path relative to current page location
    ...
    initializeTracks() {
        ...
        this.tracks = [
            ...
            { id: 'track2', name: "ok", audio: this.getAssetPath("lagu/ok.mp3"), cd: this.getAssetPath("assets/cd/cd1.webp") },
            { id: 'track3', name: "i know", audio: this.getAssetPath("lagu/i know.mp3"), cd: this.getAssetPath("assets/cd/cd1.webp") },
            { id: 'track4', name: "what i want", audio: this.getAssetPath("lagu/what i want.mp3"), cd: this.getAssetPath("assets/cd/cd1.webp") },
            { id: 'track5', name: "terus terang", audio: this.getAssetPath("lagu/terus terang.mp3"), cd: this.getAssetPath("assets/cd/cd2.webp") },
            { id: 'track6', name: "Hello World!", audio: this.getAssetPath("lagu/interlude.mp3"), cd: this.getAssetPath("assets/cd/cd1.webp") },
            { id: 'track7', name: "Bone music", audio: this.getAssetPath("lagu/test1.mp3"), cd: this.getAssetPath("assets/cd/cd3.webp") },
            { id: 'track8', name: "ADHD music", audio: this.getAssetPath("lagu/adhd.mp3"), cd: this.getAssetPath("assets/cd/cdjackass.webp") },
            { id: 'track9', name: "Fifa 2030", audio: this.getAssetPath("lagu/fifa.mp3"), cd: this.getAssetPath("assets/cd/cdfifa.webp") },
    ...
    async play() {
        ...
        try {
            ...
            if (window.analytics && this.currentTrack) {
                const track = this.tracks.find(t => t.id === this.currentTrack);
                window.analytics.trackMusicEvent('play', track ? track.name : 'unknown');
            }
            
            // Track music player performance
            if (window.speedInsights && this.currentTrack) {
                const playLatency = performance.now() - this.playStartTime;
                window.speedInsights.trackCustomMetric('music_play_latency', playLatency, 
    ...
    pause() {
        ...
        // Track music pause event
        if (window.analytics && this.currentTrack) {
            const track = this.tracks.find(t => t.id === this.currentTrack);
            window.analytics.trackMusicEvent('pause', track ? track.name : 'unknown');
        }
    }
    ...
    playNext() {
        ...
        if (window.analytics) {
            ...
            window.analytics.trackMusicEvent('next_track', nextTrack ? nextTrack.name : 'unknown');
        }
    ...
    playPrevious() {
        ...
        if (window.analytics) {
            ...
            window.analytics.trackMusicEvent('previous_track', prevTrack ? prevTrack.name : 'unknown');
        }
    ...
    renderTracklist(containerSelector) {
        ...
        if (!container) {
            console.error('🎵 Tracklist container not found:', containerSelector);
            console.error('🎵 Available elements with similar IDs:', 
                Array.from(document.querySelectorAll('[id*="track"]')).map(el => ({ id: el.id, tag: el.tagName })));
            return;
        }
        ...
        if (renderedItems.length === 0) {
            console.error('🎵 ERROR: No tracks were rendered! Debugging info:', {
                ...
                containerSelector,
                container: !!container,
                trackItemsContainer: !!trackItemsContainer,
                containerHTML: trackItemsContainer?.innerHTML?.substring(0, 200) + '...'
            });
        }
...
const persistentPlayerInstance = new PersistentMusicPlayer();

// Assign to window object with explicit check
window.persistentPlayer = persistentPlayerInstance;

// Ensure it's actually on the window
if (typeof window !== 'undefined') {
    ...
    console.log('🎵 persistentPlayer created and assigned to window:', {
        ...
        windowKeys: Object.keys(window).filter(k => k.includes('player'))
    });
} else {
    console.error('🎵 Window object not available!');
}

export default persistentPlayerInstance;
